...
namespace epmodel {
  namespace detail {
    ...
    Curve ChillerElectricEIR_Impl::coolingCapacityFunctionOfTemperature() const {
      auto target =
        getObject<ModelObject>().getModelObjectTarget<Curve>(openstudio::Chiller_Electric_EIRFields::CoolingCapacityFunctionofTemperatureCurveName);
      OS_ASSERT(target);
      return *target;
    }
    ...
    bool ChillerElectricEIR_Impl::setLeavingChilledWaterLowerTemperatureLimit(double leavingChilledWaterLowerTemperatureLimit) {
      const bool result =
        setDouble(openstudio::Chiller_Electric_EIRFields::LeavingChilledWaterLowerTemperatureLimit, leavingChilledWaterLowerTemperatureLimit);
      OS_ASSERT(result);
      return result;
    }
    ...
    bool ChillerElectricEIR_Impl::setBasinHeaterSchedule(Schedule& schedule) {
      return setSchedule(openstudio::Chiller_Electric_EIRFields::BasinHeaterOperatingScheduleName, "ChillerElectricEIR", "Basin Heater Operating",
                         schedule);
    }
