...
namespace epmodel {
  ...
  std::vector<std::string> HeatExchangerFluidToFluid::controlTypeValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(), openstudio::HeatExchanger_FluidToFluidFields::ControlType);
  }
...
namespace epmodel {
  namespace detail {
    ...
    bool HeatExchangerFluidToFluid_Impl::setAvailabilitySchedule(Schedule& schedule) {
      return setSchedule(openstudio::HeatExchanger_FluidToFluidFields::AvailabilityScheduleName, "HeatExchangerFluidToFluid", "Availability",
                         schedule);
    }
