...
namespace detail {
  ...
  boost::optional<ZoneHVACEnergyRecoveryVentilatorController> ZoneHVACEnergyRecoveryVentilator_Impl::controller() const {
    return getObject<ModelObject>().getModelObjectTarget<ZoneHVACEnergyRecoveryVentilatorController>(ZoneHVAC_EnergyRecoveryVentilatorFields::ControllerName);
  }
  ...
  std::vector<ModelObject> ZoneHVACEnergyRecoveryVentilator_Impl::children() const {
    ...
    if (auto exhaustAirFan = getObject<ModelObject>().getModelObjectTarget<ModelObject>(ZoneHVAC_EnergyRecoveryVentilatorFields::ExhaustAirFanName)) {
      result.push_back(*exhaustAirFan);
    }
    if (auto heatExchanger = getObject<ModelObject>().getModelObjectTarget<ModelObject>(ZoneHVAC_EnergyRecoveryVentilatorFields::HeatExchangerName)) {
      result.push_back(*heatExchanger);
    }
    if (auto controllerObject = getObject<ModelObject>().getModelObjectTarget<ModelObject>(ZoneHVAC_EnergyRecoveryVentilatorFields::ControllerName)) {
  ...
  bool ZoneHVACEnergyRecoveryVentilator_Impl::addToThermalZone(ThermalZone& thermalZone) {
    ...
    if (!equipmentList) {
      ...
      OS_ASSERT(connections.setPointer(ZoneHVAC_EquipmentConnectionsFields::ZoneConditioningEquipmentListName, newEquipmentList.handle()));
    ...
    if (auto connections = zoneImpl->zoneHVACEquipmentConnections()) {
      if (auto zoneInlet = connections->zoneAirInletNode()) {
        if (outlet && (*zoneInlet == *outlet)) {
          connections->setPointer(ZoneHVAC_EquipmentConnectionsFields::ZoneAirInletNodeorNodeListName, Handle());
        }
      }
      if (auto zoneReturn = connections->zoneReturnAirNode()) {
        if (inlet && (*zoneReturn == *inlet)) {
          connections->setPointer(ZoneHVAC_EquipmentConnectionsFields::ZoneReturnAirNodeorNodeListName, Handle());
        }
      }
    }
  ...
  bool ZoneHVACEnergyRecoveryVentilator_Impl::reconcileContainedAirPath(bool allowChildNodeRecovery, LoadContext* context) {
    ...
    trackNodeChange(heatExchanger->getImpl<ModelObject_Impl>()->setPointer(heatExchanger->secondaryAirInletPort(), zoneExhaustNode.handle(), false));
    trackNodeChange(
    ...
    trackNodeChange(exhaustAirFan->getImpl<ModelObject_Impl>()->setPointer(exhaustAirFan->inletPort(), exhaustAirFanInletNodeValue.handle(), false));
