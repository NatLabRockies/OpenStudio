  ...
  namespace detail {
    ...
    namespace {
      ...
      bool isEvaporativeCoolerUnitFan(const HVACComponent& component) {
        ...
        return (type == IddObjectType::Fan_ConstantVolume) || (type == IddObjectType::Fan_VariableVolume) || (type == IddObjectType::Fan_OnOff)
    ...
    HVACComponent ZoneHVACEvaporativeCoolerUnit_Impl::firstEvaporativeCooler() const {
      auto value =
        getObject<ModelObject>().getModelObjectTarget<HVACComponent>(ZoneHVAC_EvaporativeCoolerUnitFields::FirstEvaporativeCoolerObjectName);
      OS_ASSERT(value);
      return *value;
    }
    ...
    bool ZoneHVACEvaporativeCoolerUnit_Impl::reconcileContainedAirPath(bool allowChildNodeRecovery, LoadContext* context) {
      ...
      auto outdoorAirNodeValue =
        resolvedOrCreatedNodeTarget(ZoneHVAC_EvaporativeCoolerUnitFields::OutdoorAirInletNodeName, baseName + " Outdoor Air Node");
      ...
      boost::optional<Node> firstCoolerOutlet =
        keepOrCreateNode(firstEvaporativeCoolerOutletNode(), model(),
                         secondCooler ? baseName + " First Evaporative Cooler Outlet Node" : baseName + " Air Outlet Node");
      if (!secondCooler) {
      ...
      if (secondCooler) {
        secondCoolerOutlet = blowThroughFan
                               ? outletNode
                               : keepOrCreateNode(secondEvaporativeCoolerOutletNode(), model(), baseName + " Second Evaporative Cooler Outlet Node");
      }
      ...
      if (blowThroughFan) {
        ...
        trackNodeChange(firstCooler->getImpl<ModelObject_Impl>()->setPointer(
          firstCooler->outletPort(), (secondCooler ? *firstCoolerOutlet : *firstCoolerOutlet).handle(), false));
