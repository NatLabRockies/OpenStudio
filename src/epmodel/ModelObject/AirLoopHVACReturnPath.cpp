...
boost::optional<openstudio::epmodel::Node> AirLoopHVACReturnPath_Impl::returnAirPathOutletNode() const {
  if (auto node = getObject<openstudio::epmodel::AirLoopHVACReturnPath>().getModelObjectTarget<openstudio::epmodel::Node>(
        openstudio::AirLoopHVAC_ReturnPathFields::ReturnAirPathOutletNodeName)) {
    return node;
  }

  auto name = getString(openstudio::AirLoopHVAC_ReturnPathFields::ReturnAirPathOutletNodeName);
  if (name && !name->empty()) {
