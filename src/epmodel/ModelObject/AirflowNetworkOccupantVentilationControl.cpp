...
double AirflowNetworkOccupantVentilationControl_Impl::maximumPredictedPercentageofDissatisfiedThreshold() const {
  const auto value =
    getDouble(openstudio::AirflowNetwork_OccupantVentilationControlFields::MaximumThresholdforPersonsDissatisfiedPPD, true);
  OS_ASSERT(value);
  return *value;
}
...
void AirflowNetworkOccupantVentilationControl_Impl::setOccupancyCheck(bool occupancyCheck) {
  const bool result =
    setString(openstudio::AirflowNetwork_OccupantVentilationControlFields::OccupancyCheck, occupancyCheck ? "Yes" : "No");
  OS_ASSERT(result);
}
