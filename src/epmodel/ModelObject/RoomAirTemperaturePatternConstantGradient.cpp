...
bool RoomAirTemperaturePatternConstantGradient_Impl::setControlIntegerforPatternControlScheduleName(int controlIntegerforPatternControlScheduleName) {
  const bool result =
    setInt(openstudio::RoomAir_TemperaturePattern_ConstantGradientFields::ControlIntegerforPatternControlScheduleName,
           controlIntegerforPatternControlScheduleName);
  OS_ASSERT(result);
  return result;
}
