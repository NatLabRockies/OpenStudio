...
namespace epmodel {
  ...
  class EPMODEL_API AirLoopHVACOutdoorAirSystem : public HVACComponent
  {
   public:
    ...
    // - Evidence: `src/model/AirLoopHVACOutdoorAirSystem.hpp`, `src/energyplus/ForwardTranslator/ForwardTranslateAirLoopHVACOutdoorAirSystem.cpp`, `src/epmodel/test/AirLoopHVACOutdoorAirSystem_GTest.cpp`, `src/epmodel/test/AirLoopHVACDedicatedOutdoorAirSystem_GTest.cpp`, and `resources/Examples/compact_osw/epmodel_relief_return_small_office_idf.osw` show the canonical API shape, node-level topology, persistence, controller projection, and an executable relief-versus-return lifecycle.
