...
namespace epmodel {
  ...
  GroundHeatTransferControl::GroundHeatTransferControl(std::shared_ptr<detail::GroundHeatTransferControl_Impl> impl) : ModelObject(std::move(impl)) {}
...
namespace epmodel {
  namespace detail {
    ...
    bool GroundHeatTransferControl_Impl::setRunBasementPreprocessor(bool runBasementPreprocessor) {
      const bool result = setString(openstudio::GroundHeatTransfer_ControlFields::RunBasementPreprocessor, runBasementPreprocessor ? "Yes" : "No");
      OS_ASSERT(result);
      return result;
    }
