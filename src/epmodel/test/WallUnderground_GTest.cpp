...
TEST_F(EPModelFixture, API_WallUnderground_DefaultConstructor) {
  Model model;
  WallUnderground wall(model);
  EXPECT_EQ(WallUnderground::iddObjectType(), wall.iddObject().type());
  EXPECT_DOUBLE_EQ(90.0, wall.tiltAngle());
...
TEST_F(EPModelFixture, WallUnderground_ScalarAccessors_RoundTrip) {
  Model model;
  WallUnderground wall(model);
