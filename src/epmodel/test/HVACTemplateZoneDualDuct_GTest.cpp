...
TEST_F(EPModelFixture, HVACTemplateZoneDualDuct_DefaultConstructor) {
  Model model;
  HVACTemplateZoneDualDuct object(model);
  EXPECT_EQ(HVACTemplateZoneDualDuct::iddObjectType(), object.iddObject().type());
}

TEST_F(EPModelFixture, HVACTemplateZoneDualDuct_ScalarAccessors_RoundTrip) {
  Model model;
  HVACTemplateZoneDualDuct object(model);
  ...
  EXPECT_TRUE(object.isZoneMinimumAirFlowFractionDefaulted());
  EXPECT_TRUE(object.setZoneMinimumAirFlowFraction(0.3));
  EXPECT_DOUBLE_EQ(0.3, object.zoneMinimumAirFlowFraction());
  object.resetZoneMinimumAirFlowFraction();
  EXPECT_TRUE(object.isZoneMinimumAirFlowFractionDefaulted());
  EXPECT_FALSE(object.setZoneMinimumAirFlowFraction(1.2));
  ...
  EXPECT_GE(HVACTemplateZoneDualDuct::outdoorAirMethodValues().size(), 1u);
  EXPECT_GE(HVACTemplateZoneDualDuct::baseboardHeatingTypeValues().size(), 1u);
  EXPECT_GE(HVACTemplateZoneDualDuct::zoneCoolingDesignSupplyAirTemperatureInputMethodValues().size(), 1u);
  EXPECT_GE(HVACTemplateZoneDualDuct::zoneHeatingDesignSupplyAirTemperatureInputMethodValues().size(), 1u);
}
