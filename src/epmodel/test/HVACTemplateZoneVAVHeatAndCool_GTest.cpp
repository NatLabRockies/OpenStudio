...
#include "../ModelObject/HVACTemplateZoneVAVHeatAndCool.hpp"

using namespace openstudio::epmodel;

TEST_F(EPModelFixture, HVACTemplateZoneVAVHeatAndCool_DefaultConstructor) {
  Model model;
  HVACTemplateZoneVAVHeatAndCool object(model);
  EXPECT_EQ(HVACTemplateZoneVAVHeatAndCool::iddObjectType(), object.iddObject().type());
}

TEST_F(EPModelFixture, HVACTemplateZoneVAVHeatAndCool_ScalarAccessors_RoundTrip) {
  Model model;
  HVACTemplateZoneVAVHeatAndCool object(model);
  ...
  EXPECT_TRUE(object.isConstantMinimumAirFlowFractionDefaulted());
  EXPECT_DOUBLE_EQ(0.2, object.constantMinimumAirFlowFraction());
  ...
  object.resetConstantMinimumAirFlowFraction();
  EXPECT_TRUE(object.isConstantMinimumAirFlowFractionDefaulted());
  ...
  EXPECT_TRUE(object.isReheatCoilTypeDefaulted());
  EXPECT_TRUE(object.setReheatCoilType("Gas"));
  EXPECT_EQ("Gas", object.reheatCoilType());
  EXPECT_FALSE(object.setReheatCoilType("InvalidChoice"));
  object.resetReheatCoilType();
  EXPECT_TRUE(object.isReheatCoilTypeDefaulted());

  EXPECT_FALSE(object.maximumReheatAirTemperature());
  EXPECT_TRUE(object.setMaximumReheatAirTemperature(35.0));
  ASSERT_TRUE(object.maximumReheatAirTemperature());
  EXPECT_DOUBLE_EQ(35.0, object.maximumReheatAirTemperature().get());
  object.resetMaximumReheatAirTemperature();
  EXPECT_FALSE(object.maximumReheatAirTemperature());
  EXPECT_FALSE(object.setMaximumReheatAirTemperature(0.0));
  ...
  EXPECT_GE(HVACTemplateZoneVAVHeatAndCool::outdoorAirMethodValues().size(), 1u);
  EXPECT_GE(HVACTemplateZoneVAVHeatAndCool::reheatCoilTypeValues().size(), 1u);
  EXPECT_GE(HVACTemplateZoneVAVHeatAndCool::baseboardHeatingTypeValues().size(), 1u);
  EXPECT_GE(HVACTemplateZoneVAVHeatAndCool::zoneCoolingDesignSupplyAirTemperatureInputMethodValues().size(), 1u);
  EXPECT_GE(HVACTemplateZoneVAVHeatAndCool::zoneHeatingDesignSupplyAirTemperatureInputMethodValues().size(), 1u);
}
