...
#include "../ModelObject/FaultModelTemperatureSensorOffsetCoilSupplyAir.hpp"

using namespace openstudio::epmodel;

TEST_F(EPModelFixture, FaultModelTemperatureSensorOffsetCoilSupplyAir_DefaultConstructor) {
  Model model;
  FaultModelTemperatureSensorOffsetCoilSupplyAir fault(model);
  EXPECT_EQ(FaultModelTemperatureSensorOffsetCoilSupplyAir::iddObjectType(), fault.iddObject().type());

  EXPECT_TRUE(fault.isReferenceSensorOffsetDefaulted());
  EXPECT_DOUBLE_EQ(0.0, fault.referenceSensorOffset());
}

TEST_F(EPModelFixture, FaultModelTemperatureSensorOffsetCoilSupplyAir_ScalarAccessors_RoundTrip) {
  Model model;
  FaultModelTemperatureSensorOffsetCoilSupplyAir fault(model);
