...
TEST_F(EPModelFixture, CurveCubicLinear_DefaultConstructor) {
  Model model;
  CurveCubicLinear curve(model);
  EXPECT_EQ(CurveCubicLinear::iddObjectType(), curve.iddObject().type());
  ...
  EXPECT_DOUBLE_EQ(0.0, curve.coefficient5y());
...
TEST_F(EPModelFixture, CurveCubicLinear_ScalarAccessors_RoundTrip) {
  Model model;
  CurveCubicLinear curve(model);
  ...
  const auto xValues = CurveCubicLinear::validInputUnitTypeforXValues();
  ...
  const auto yValues = CurveCubicLinear::validInputUnitTypeforYValues();
  ...
  const auto outputValues = CurveCubicLinear::validOutputUnitTypeValues();
