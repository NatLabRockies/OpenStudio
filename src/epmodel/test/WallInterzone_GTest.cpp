...
TEST_F(EPModelFixture, API_WallInterzone_DefaultConstructor) {
  Model model;
  WallInterzone wall(model);
  EXPECT_EQ(WallInterzone::iddObjectType(), wall.iddObject().type());
...
TEST_F(EPModelFixture, WallInterzone_ScalarAccessors_RoundTrip) {
  Model model;
  WallInterzone wall(model);
