...
TEST_F(EPModelFixture, CurveQuartic_DefaultConstructor) {
  Model model;
  CurveQuartic curve(model);
  EXPECT_EQ(CurveQuartic::iddObjectType(), curve.iddObject().type());
  ...
  EXPECT_DOUBLE_EQ(0.0, curve.coefficient4xPOW3());
  EXPECT_DOUBLE_EQ(1.0, curve.coefficient5xPOW4());
...
TEST_F(EPModelFixture, CurveQuartic_ScalarAccessors_RoundTrip) {
  Model model;
  CurveQuartic curve(model);
  ...
  EXPECT_TRUE(curve.setCoefficient4xPOW3(4.4));
  EXPECT_TRUE(curve.setCoefficient5xPOW4(5.5));
  ...
  EXPECT_DOUBLE_EQ(4.4, curve.coefficient4xPOW3());
  EXPECT_DOUBLE_EQ(5.5, curve.coefficient5xPOW4());
  ...
  const auto xValues = CurveQuartic::validInputUnitTypeforXValues();
  ...
  const auto outputValues = CurveQuartic::validOutputUnitTypeValues();
