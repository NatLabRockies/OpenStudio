...
#include "../ModelObject/FloorInterzone.hpp"

using namespace openstudio::epmodel;

TEST_F(EPModelFixture, FloorInterzone_DefaultConstructor) {
  Model model;
  FloorInterzone floor(model);
  EXPECT_EQ(FloorInterzone::iddObjectType(), floor.iddObject().type());
...
TEST_F(EPModelFixture, FloorInterzone_ScalarAccessors_RoundTrip) {
  Model model;
  FloorInterzone floor(model);
  ...
  EXPECT_TRUE(floor.setAzimuthAngle(0.0));
  EXPECT_TRUE(floor.setAzimuthAngle(360.0));
  EXPECT_FALSE(floor.setAzimuthAngle(-0.1));
  EXPECT_FALSE(floor.setAzimuthAngle(360.1));
  ASSERT_TRUE(floor.azimuthAngle());
  EXPECT_DOUBLE_EQ(360.0, floor.azimuthAngle().get());

  EXPECT_TRUE(floor.setTiltAngle(170.0));
  EXPECT_FALSE(floor.isTiltAngleDefaulted());
  EXPECT_DOUBLE_EQ(170.0, floor.tiltAngle());
  EXPECT_TRUE(floor.setTiltAngle(0.0));
  EXPECT_TRUE(floor.setTiltAngle(180.0));
