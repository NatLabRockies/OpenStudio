...
TEST_F(EPModelFixture, HVACTemplateZoneConstantVolume_DefaultConstructor) {
  Model model;
  HVACTemplateZoneConstantVolume object(model);
  EXPECT_EQ(HVACTemplateZoneConstantVolume::iddObjectType(), object.iddObject().type());
}

TEST_F(EPModelFixture, HVACTemplateZoneConstantVolume_ScalarAccessors_RoundTrip) {
  Model model;
  HVACTemplateZoneConstantVolume object(model);
  ...
  EXPECT_GE(HVACTemplateZoneConstantVolume::outdoorAirMethodValues().size(), 1u);
  EXPECT_GE(HVACTemplateZoneConstantVolume::reheatCoilTypeValues().size(), 1u);
  EXPECT_GE(HVACTemplateZoneConstantVolume::baseboardHeatingTypeValues().size(), 1u);
  EXPECT_GE(HVACTemplateZoneConstantVolume::zoneCoolingDesignSupplyAirTemperatureInputMethodValues().size(), 1u);
  EXPECT_GE(HVACTemplateZoneConstantVolume::zoneHeatingDesignSupplyAirTemperatureInputMethodValues().size(), 1u);
}
