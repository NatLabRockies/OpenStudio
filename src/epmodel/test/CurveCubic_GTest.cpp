...
TEST_F(EPModelFixture, CurveCubic_DefaultConstructor) {
  Model model;
  CurveCubic curve(model);
  EXPECT_EQ(CurveCubic::iddObjectType(), curve.iddObject().type());
  ...
  EXPECT_DOUBLE_EQ(1.0, curve.coefficient4xPOW3());
...
TEST_F(EPModelFixture, CurveCubic_ScalarAccessors_RoundTrip) {
  Model model;
  CurveCubic curve(model);
  ...
  const auto xValues = CurveCubic::validInputUnitTypeforXValues();
  ...
  const auto outputValues = CurveCubic::validOutputUnitTypeValues();
