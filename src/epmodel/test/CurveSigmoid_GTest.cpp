...
TEST_F(EPModelFixture, CurveSigmoid_DefaultConstructor) {
  Model model;
  CurveSigmoid curve(model);
  EXPECT_EQ(CurveSigmoid::iddObjectType(), curve.iddObject().type());
  ...
  EXPECT_DOUBLE_EQ(1.0, curve.coefficient5C5());
...
TEST_F(EPModelFixture, CurveSigmoid_ScalarAccessors_RoundTrip) {
  Model model;
  CurveSigmoid curve(model);

  EXPECT_TRUE(curve.setCoefficient1C1(0.1));
  ...
  EXPECT_DOUBLE_EQ(0.1, curve.coefficient1C1());
  ...
  const auto xValues = CurveSigmoid::validInputUnitTypeforxValues();
  ...
  const auto outputValues = CurveSigmoid::validOutputUnitTypeValues();
