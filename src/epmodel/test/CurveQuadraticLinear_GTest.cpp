...
TEST_F(EPModelFixture, CurveQuadraticLinear_DefaultConstructor) {
  Model model;
  CurveQuadraticLinear curve(model);
  EXPECT_EQ(CurveQuadraticLinear::iddObjectType(), curve.iddObject().type());
  ...
  EXPECT_DOUBLE_EQ(0.0, curve.coefficient5xTIMESY());
  EXPECT_DOUBLE_EQ(1.0, curve.coefficient6xPOW2TIMESY());
...
TEST_F(EPModelFixture, CurveQuadraticLinear_ScalarAccessors_RoundTrip) {
  Model model;
  CurveQuadraticLinear curve(model);
  ...
  EXPECT_TRUE(curve.setCoefficient5xTIMESY(5.5));
  EXPECT_TRUE(curve.setCoefficient6xPOW2TIMESY(6.6));
  EXPECT_TRUE(curve.setMinimumValueofx(-2.0));
  EXPECT_TRUE(curve.setMaximumValueofx(2.0));
  EXPECT_TRUE(curve.setMinimumValueofy(-3.0));
  EXPECT_TRUE(curve.setMaximumValueofy(3.0));
  ...
  EXPECT_DOUBLE_EQ(5.5, curve.coefficient5xTIMESY());
  EXPECT_DOUBLE_EQ(6.6, curve.coefficient6xPOW2TIMESY());
  EXPECT_DOUBLE_EQ(-2.0, curve.minimumValueofx());
  EXPECT_DOUBLE_EQ(2.0, curve.maximumValueofx());
  EXPECT_DOUBLE_EQ(-3.0, curve.minimumValueofy());
  EXPECT_DOUBLE_EQ(3.0, curve.maximumValueofy());
  ...
  const auto xValues = CurveQuadraticLinear::validInputUnitTypeforXValues();
  ...
  const auto yValues = CurveQuadraticLinear::validInputUnitTypeforYValues();
  ...
  const auto outputValues = CurveQuadraticLinear::validOutputUnitTypeValues();
