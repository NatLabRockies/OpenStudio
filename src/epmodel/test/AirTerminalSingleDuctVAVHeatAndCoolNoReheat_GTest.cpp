...
TEST_F(EPModelFixture, AirTerminalSingleDuctVAVHeatAndCoolNoReheat_AddToNode_RejectsInvalidNodesAndDuplicateZoneInsertion) {
  ...
  EXPECT_EQ(zoneInletNode, *terminalOutletNode);
...
TEST_F(EPModelFixture, AirTerminalSingleDuctVAVHeatAndCoolNoReheat_AddToNode_LateFailureIsAtomicAndRetryable) {
  ...
  EXPECT_EQ(zoneInletNode.cast<ModelObject>(), *airLoop.zoneSplitter().lastOutletModelObject());
...
TEST_F(EPModelFixture, AirTerminalSingleDuctVAVHeatAndCoolNoReheat_Remove_ReconnectsZoneBranchAndCleansZoneReferences) {
  ...
  auto zoneInletObject = terminalForRemoveFromLoop.outletModelObject();
  ...
  ASSERT_TRUE(terminalForRemove.outletModelObject());
  EXPECT_EQ(zoneInletNode->handle(), terminalForRemove.outletModelObject()->handle());
