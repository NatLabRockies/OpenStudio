...
TEST_F(EPModelFixture, CoilCoolingLowTempRadiantVarFlow_ScalarsWriteThroughParentAndDesignObject) {
  ...
  EXPECT_DOUBLE_EQ(
    0.8,
    design->getDouble(openstudio::ZoneHVAC_LowTemperatureRadiant_VariableFlow_DesignFields::FractionofAutosizedCoolingDesignCapacity, true).get());
  ...
  EXPECT_FALSE(design->getTarget(openstudio::ZoneHVAC_LowTemperatureRadiant_VariableFlow_DesignFields::CoolingControlTemperatureScheduleName));
