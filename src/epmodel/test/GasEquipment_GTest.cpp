...
TEST_F(EPModelFixture, GasEquipment_DefaultConstructor) {
  Model model;
  GasEquipment object(model);
  EXPECT_EQ(GasEquipment::iddObjectType(), object.iddObject().type());
  EXPECT_FALSE(object.nameString().empty());
}

TEST_F(EPModelFixture, GasEquipment_ScalarAccessors_RoundTrip) {
  Model model;
  GasEquipment object(model);
  ...
  EXPECT_TRUE(object.setCarbonDioxideGenerationRate(1.0e-8));
  EXPECT_DOUBLE_EQ(1.0e-8, object.carbonDioxideGenerationRate());
  object.resetCarbonDioxideGenerationRate();
  EXPECT_TRUE(object.isCarbonDioxideGenerationRateDefaulted());
  EXPECT_DOUBLE_EQ(0.0, object.carbonDioxideGenerationRate());
