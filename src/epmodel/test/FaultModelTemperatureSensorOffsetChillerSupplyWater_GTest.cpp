...
TEST_F(EPModelFixture, FaultModelTemperatureSensorOffsetChillerSupplyWater_DefaultConstructor) {
  Model model;
  FaultModelTemperatureSensorOffsetChillerSupplyWater fault(model);
  EXPECT_EQ(FaultModelTemperatureSensorOffsetChillerSupplyWater::iddObjectType(), fault.iddObject().type());

  EXPECT_TRUE(fault.isReferenceSensorOffsetDefaulted());
  EXPECT_DOUBLE_EQ(0.0, fault.referenceSensorOffset());
}

TEST_F(EPModelFixture, FaultModelTemperatureSensorOffsetChillerSupplyWater_ScalarAccessors_RoundTrip) {
  Model model;
  FaultModelTemperatureSensorOffsetChillerSupplyWater fault(model);
