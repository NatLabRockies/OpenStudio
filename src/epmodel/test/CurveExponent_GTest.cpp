...
TEST_F(EPModelFixture, CurveExponent_DefaultConstructor) {
  Model model;
  CurveExponent curve(model);
  EXPECT_EQ(CurveExponent::iddObjectType(), curve.iddObject().type());

  EXPECT_DOUBLE_EQ(0.0, curve.coefficient1Constant());
  EXPECT_DOUBLE_EQ(1.0, curve.coefficient2Constant());
  EXPECT_DOUBLE_EQ(1.0, curve.coefficient3Constant());
...
TEST_F(EPModelFixture, CurveExponent_ScalarAccessors_RoundTrip) {
  Model model;
  CurveExponent curve(model);

  EXPECT_TRUE(curve.setCoefficient1Constant(1.1));
  EXPECT_TRUE(curve.setCoefficient2Constant(2.2));
  EXPECT_TRUE(curve.setCoefficient3Constant(3.3));
  ...
  EXPECT_DOUBLE_EQ(2.2, curve.coefficient2Constant());
  EXPECT_DOUBLE_EQ(3.3, curve.coefficient3Constant());
  ...
  const auto xValues = CurveExponent::validInputUnitTypeforXValues();
  ...
  const auto outputValues = CurveExponent::validOutputUnitTypeValues();
