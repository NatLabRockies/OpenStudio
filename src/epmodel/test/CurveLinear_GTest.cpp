...
TEST_F(EPModelFixture, CurveLinear_DefaultConstructor) {
  Model model;
...
TEST_F(EPModelFixture, CurveLinear_ScalarAccessors_RoundTrip) {
  Model model;
  CurveLinear curve(model);
  ...
  const auto xValues = CurveLinear::validInputUnitTypeforXValues();
  ...
  const auto outputValues = CurveLinear::validOutputUnitTypeValues();
