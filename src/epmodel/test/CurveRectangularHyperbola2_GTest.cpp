...
TEST_F(EPModelFixture, CurveRectangularHyperbola2_DefaultConstructor) {
  Model model;
  CurveRectangularHyperbola2 curve(model);
  EXPECT_EQ(CurveRectangularHyperbola2::iddObjectType(), curve.iddObject().type());
...
TEST_F(EPModelFixture, CurveRectangularHyperbola2_ScalarAccessors_RoundTrip) {
  Model model;
  CurveRectangularHyperbola2 curve(model);
  ...
  const auto xValues = CurveRectangularHyperbola2::validInputUnitTypeforxValues();
  ...
  const auto outputValues = CurveRectangularHyperbola2::validOutputUnitTypeValues();
