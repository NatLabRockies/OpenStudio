...
TEST_F(EPModelFixture, CurveExponentialDecay_DefaultConstructor) {
  Model model;
  CurveExponentialDecay curve(model);
  EXPECT_EQ(CurveExponentialDecay::iddObjectType(), curve.iddObject().type());
...
TEST_F(EPModelFixture, CurveExponentialDecay_ScalarAccessors_RoundTrip) {
  Model model;
  CurveExponentialDecay curve(model);
  ...
  const auto xValues = CurveExponentialDecay::validInputUnitTypeforxValues();
  ...
  const auto outputValues = CurveExponentialDecay::validOutputUnitTypeValues();
