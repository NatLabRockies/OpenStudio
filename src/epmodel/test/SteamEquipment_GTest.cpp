...
TEST_F(EPModelFixture, SteamEquipment_DefaultConstructor) {
  Model model;
  SteamEquipment object(model);
  EXPECT_EQ(SteamEquipment::iddObjectType(), object.iddObject().type());
  EXPECT_FALSE(object.nameString().empty());
}

TEST_F(EPModelFixture, SteamEquipment_ScalarAccessors_RoundTrip) {
  Model model;
  SteamEquipment object(model);
