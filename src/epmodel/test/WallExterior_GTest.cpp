...
TEST_F(EPModelFixture, API_WallExterior_DefaultConstructor) {
  Model model;
  WallExterior wall(model);
  EXPECT_EQ(WallExterior::iddObjectType(), wall.iddObject().type());
...
TEST_F(EPModelFixture, WallExterior_ScalarAccessors_RoundTrip) {
  Model model;
  WallExterior wall(model);
