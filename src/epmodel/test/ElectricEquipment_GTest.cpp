...
TEST_F(EPModelFixture, ElectricEquipment_DefaultConstructor) {
  Model model;
  ElectricEquipment object(model);
  EXPECT_EQ(ElectricEquipment::iddObjectType(), object.iddObject().type());
  EXPECT_FALSE(object.nameString().empty());
}

TEST_F(EPModelFixture, ElectricEquipment_ScalarAccessors_RoundTrip) {
  Model model;
  ElectricEquipment object(model);
