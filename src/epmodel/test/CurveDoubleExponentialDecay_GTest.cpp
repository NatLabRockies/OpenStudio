...
TEST_F(EPModelFixture, CurveDoubleExponentialDecay_DefaultConstructor) {
  Model model;
  CurveDoubleExponentialDecay curve(model);
  EXPECT_EQ(CurveDoubleExponentialDecay::iddObjectType(), curve.iddObject().type());
...
TEST_F(EPModelFixture, CurveDoubleExponentialDecay_ScalarAccessors_RoundTrip) {
  Model model;
  CurveDoubleExponentialDecay curve(model);
  ...
  const auto xValues = CurveDoubleExponentialDecay::validInputUnitTypeforxValues();
  ...
  const auto outputValues = CurveDoubleExponentialDecay::validOutputUnitTypeValues();
