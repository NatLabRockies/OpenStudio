...
namespace epmodel {
  namespace detail {
    ...
    Schedule CoilHeatingElectricMultiStage_Impl::availabilitySchedule() const {
      auto value =
        getObject<ModelObject>().getModelObjectTarget<Schedule>(openstudio::Coil_Heating_Electric_MultiStageFields::AvailabilityScheduleName);
      if (!value) {
        value = this->model().alwaysOnDiscreteSchedule();
        OS_ASSERT(value);
        const_cast<CoilHeatingElectricMultiStage_Impl*>(this)->setAvailabilitySchedule(*value);
        value = getObject<ModelObject>().getModelObjectTarget<Schedule>(openstudio::Coil_Heating_Electric_MultiStageFields::AvailabilityScheduleName);
      }
      OS_ASSERT(value);
      return *value;
    }
