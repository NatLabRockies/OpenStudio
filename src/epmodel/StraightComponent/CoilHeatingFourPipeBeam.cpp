...
namespace epmodel {
  ...
  CoilHeatingFourPipeBeam::CoilHeatingFourPipeBeam(const Model& model) : StraightComponent(CoilHeatingFourPipeBeam::iddObjectType(), model) {}

  CoilHeatingFourPipeBeam::CoilHeatingFourPipeBeam(std::shared_ptr<detail::CoilHeatingFourPipeBeam_Impl> impl) : StraightComponent(std::move(impl)) {}

  IddObjectType CoilHeatingFourPipeBeam::iddObjectType() {
    return IddObjectType::OS_Coil_Heating_FourPipeBeam;
  }

  namespace detail {

    unsigned CoilHeatingFourPipeBeam_Impl::inletPort() const {
      return openstudio::OS_Coil_Heating_FourPipeBeamFields::HotWaterInletNodeName;
    }

    unsigned CoilHeatingFourPipeBeam_Impl::outletPort() const {
      return openstudio::OS_Coil_Heating_FourPipeBeamFields::HotWaterOutletNodeName;
    }

    boost::optional<openstudio::epmodel::PlantLoop> CoilHeatingFourPipeBeam_Impl::plantLoop() const {
