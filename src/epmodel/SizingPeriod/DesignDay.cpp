...
namespace epmodel {
  ...
  std::vector<std::string> DesignDay::validBeginEnvironmentResetModeValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(),
                          openstudio::SizingPeriod_DesignDayFields::BeginEnvironmentResetMode);
  }
...
namespace epmodel {
  namespace detail {
    ...
    bool DesignDay_Impl::setDaylightSavingTimeIndicator(bool daylightSavingTimeIndicator) {
      const bool result =
        setString(openstudio::SizingPeriod_DesignDayFields::DaylightSavingTimeIndicator, daylightSavingTimeIndicator ? "Yes" : "No");
      OS_ASSERT(result);
      return result;
    }
