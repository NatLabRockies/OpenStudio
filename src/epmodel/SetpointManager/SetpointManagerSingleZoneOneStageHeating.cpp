...
namespace epmodel {
  ...
  bool SetpointManagerSingleZoneOneStageHeating::setHeatingStageOnSupplyAirSetpointTemperature(double heatingStageOnSupplyAirSetpointTemperature) {
    const bool result = getImpl<detail::SetpointManagerSingleZoneOneStageHeating_Impl>()->setHeatingStageOnSupplyAirSetpointTemperature(
  ...
  bool SetpointManagerSingleZoneOneStageHeating::setHeatingStageOffSupplyAirSetpointTemperature(double heatingStageOffSupplyAirSetpointTemperature) {
    const bool result = getImpl<detail::SetpointManagerSingleZoneOneStageHeating_Impl>()->setHeatingStageOffSupplyAirSetpointTemperature(
    ...
    void SetpointManagerSingleZoneOneStageHeating_Impl::doCanonicalize(LoadContext& context) {
      ...
      if (auto value = getDouble(openstudio::SetpointManager_SingleZone_OneStageHeatingFields::HeatingStageOnSupplyAirSetpointTemperature, true)) {
        ...
        detail::addLoadInfo(context,
                            "Set default Heating Stage On Supply Air Setpoint Temperature to 99 for SetpointManager:SingleZone:OneStageHeating '"
      ...
      if (auto value = getDouble(openstudio::SetpointManager_SingleZone_OneStageHeatingFields::HeatingStageOffSupplyAirSetpointTemperature, true)) {
        ...
        detail::addLoadInfo(context,
                            "Set default Heating Stage Off Supply Air Setpoint Temperature to -99 for SetpointManager:SingleZone:OneStageHeating '"
                              + getObject<ModelObject>().nameString() + "'.");
      }
    }
