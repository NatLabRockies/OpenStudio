...
bool SetpointManagerSingleZoneOneStageCooling::setCoolingStageOnSupplyAirSetpointTemperature(double coolingStageOnSupplyAirSetpointTemperature) {
  const bool result =
    getImpl<detail::SetpointManagerSingleZoneOneStageCooling_Impl>()->setCoolingStageOnSupplyAirSetpointTemperature(
...
bool SetpointManagerSingleZoneOneStageCooling::setCoolingStageOffSupplyAirSetpointTemperature(
  double coolingStageOffSupplyAirSetpointTemperature) {
  const bool result =
    getImpl<detail::SetpointManagerSingleZoneOneStageCooling_Impl>()->setCoolingStageOffSupplyAirSetpointTemperature(
...
void SetpointManagerSingleZoneOneStageCooling_Impl::doCanonicalize(LoadContext& context) {
  ...
  if (auto value = getDouble(openstudio::SetpointManager_SingleZone_OneStageCoolingFields::CoolingStageOnSupplyAirSetpointTemperature, true)) {
    ...
    detail::addLoadInfo(
      context, "Set default Cooling Stage On Supply Air Setpoint Temperature to -99 for SetpointManager:SingleZone:OneStageCooling '"
  ...
  if (auto value = getDouble(openstudio::SetpointManager_SingleZone_OneStageCoolingFields::CoolingStageOffSupplyAirSetpointTemperature, true)) {
    ...
    detail::addLoadInfo(
      context, "Set default Cooling Stage Off Supply Air Setpoint Temperature to 99 for SetpointManager:SingleZone:OneStageCooling '"
                 + getObject<ModelObject>().nameString() + "'.");
  }
}
