...
namespace epmodel {
  ...
  class EPMODEL_API AirLoopHVAC : public Loop
  {
   public:
    ...
    // - Implemented Parity: `designSupplyAirFlowRate`, `designReturnAirFlowFractionofSupplyAirFlow`, supply/demand node accessors, `zoneSplitter`, `zoneMixer`, `supplyComponents`, `demandComponents`, `airLoopHVACOutdoorAirSystem`, `thermalZones`, and availability-manager APIs preserve the main single-duct loop-topology contract used by canonical model code. Implicit `addBranchForZone` clone-last supports the canonical childless, reheat, PIU, cooled-beam, four-pipe-beam, and four-pipe-induction families, plus the epmodel variable-speed-fan reheat terminal, including owned-child and plant reconnection semantics.
    // - Documented Delta: Public dual-duct helpers (`isDualDuct`, supply splitter helpers, multi-splitter surfaces) are still not exposed. Inlet-side mixers are rejected before mutation because their secondary inlet can be owned by downstream ZoneHVAC equipment, and user-defined terminals are rejected because no canonical ownership contract exists for their optional plant/program/tank relationships.
