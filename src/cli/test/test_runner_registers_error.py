...
def test_classic_workflow_removed(osclipath):
    result = subprocess.run(
        [osclipath, "classic", "run"], capture_output=True, encoding="utf-8"
    )
    ...
    assert (
        result.stderr
        == "The classic workflow runner is not available in OpenStudio 4.0 because ModelMeasures now use EPModel. "
        "Use `openstudio run ...` instead.\n"
    )


def test_reportingmeasure(osclipath):
...
"""
    ...
    )  # ReportingMeasures after the one that failed aren't run
    assert "Gathering reports" in run_log  # We still do Post Processing


def test_modelmeasure(osclipath):
    runDir, r = run_workflow(
        osclipath=osclipath,
