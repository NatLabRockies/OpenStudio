...
TEST_F(SqlFileFixture, AnnualTotalCosts) {
  ...
  // SqlResults ep_2610 = {191876887.69, 27893.06, 407.55, 361.09, 776.63, 3328949.36, 188518500.0};
  SqlResults ep_2620 = {191887151.1, 27864.41, 407.56, 360.97, 776.65, 3328941.51, 188528800.0};
  ...
  // Total annual costs for all fuel types
  EXPECT_NEAR(ep_2620.annualTotalUtilityCost, sqlFile2.annualTotalUtilityCost().get(), 0.03);

  // Costs by fuel type
  EXPECT_DOUBLE_EQ(ep_2620.annualTotalCost_Electricity, sqlFile2.annualTotalCost(FuelType::Electricity).get());
  EXPECT_DOUBLE_EQ(ep_2620.annualTotalCost_Gas, sqlFile2.annualTotalCost(FuelType::Gas).get());
  EXPECT_DOUBLE_EQ(ep_2620.annualTotalCost_DistrictCooling, sqlFile2.annualTotalCost(FuelType::DistrictCooling).get());
  EXPECT_DOUBLE_EQ(ep_2620.annualTotalCost_DistrictHeating, sqlFile2.annualTotalCost(FuelType::DistrictHeating).get());
  EXPECT_NEAR(ep_2620.annualTotalCost_Water, sqlFile2.annualTotalCost(FuelType::Water).get(), 0.03);
  EXPECT_DOUBLE_EQ(ep_2620.annualTotalCost_FuelOil_1, sqlFile2.annualTotalCost(FuelType::FuelOil_1).get());
...
TEST_F(SqlFileFixture, Regressions) {
  ...
  regressionTestSqlFile("1ZoneEvapCooler-V25-2-0.sql", 43.28, 20, 20);
  regressionTestSqlFile("1ZoneEvapCooler-V26-1-0.sql", 43.28, 20, 20);
}
