 ...
 * on the Branches
 */
TEST_F(EnergyPlusFixture, ForwardTranslator_ChillerElectricEIR) {
  ...
  EXPECT_TRUE(ch.setReferenceEnteringCondenserFluidTemperature(30.0));
  EXPECT_TRUE(ch.setReferenceChilledWaterFlowRate(3.0));
  EXPECT_TRUE(ch.setReferenceCondenserFluidFlowRate(4.0));
  ...
  EXPECT_TRUE(ch.setCondenserFanPowerRatio(1.1));
  ...
  EXPECT_TRUE(ch.setBasinHeaterCapacity(2000.0));
...

  {
    ...
    std::vector<WorkspaceObject> woChs = w.getObjectsByType(IddObjectType::Chiller_Electric_EIR);
    ASSERT_EQ(1, woChs.size());
    auto& woCh = woChs.front();

    EXPECT_EQ("My ChillerElectricEIR", woCh.getString(Chiller_Electric_EIRFields::Name).get());
    EXPECT_EQ(30000.0, woCh.getDouble(Chiller_Electric_EIRFields::ReferenceCapacity).get());
    EXPECT_EQ(4.5, woCh.getDouble(Chiller_Electric_EIRFields::ReferenceCOP).get());
    EXPECT_EQ(7.0, woCh.getDouble(Chiller_Electric_EIRFields::ReferenceLeavingChilledWaterTemperature).get());
    EXPECT_EQ(30.0, woCh.getDouble(Chiller_Electric_EIRFields::ReferenceEnteringCondenserFluidTemperature).get());
    EXPECT_EQ(3.0, woCh.getDouble(Chiller_Electric_EIRFields::ReferenceChilledWaterFlowRate).get());
    EXPECT_EQ(4.0, woCh.getDouble(Chiller_Electric_EIRFields::ReferenceCondenserFluidFlowRate).get());
    EXPECT_EQ("capfT", woCh.getString(Chiller_Electric_EIRFields::CoolingCapacityFunctionofTemperatureCurveName).get());
    EXPECT_EQ("EIRfT", woCh.getString(Chiller_Electric_EIRFields::ElectricInputtoCoolingOutputRatioFunctionofTemperatureCurveName).get());
    EXPECT_EQ("EIRfPLR", woCh.getString(Chiller_Electric_EIRFields::ElectricInputtoCoolingOutputRatioFunctionofPartLoadRatioCurveName).get());
    EXPECT_EQ(0.1, woCh.getDouble(Chiller_Electric_EIRFields::MinimumPartLoadRatio).get());
    EXPECT_EQ(1.3, woCh.getDouble(Chiller_Electric_EIRFields::MaximumPartLoadRatio).get());
    EXPECT_EQ(0.8, woCh.getDouble(Chiller_Electric_EIRFields::OptimumPartLoadRatio).get());
    EXPECT_EQ(0.2, woCh.getDouble(Chiller_Electric_EIRFields::MinimumUnloadingRatio).get());
    EXPECT_EQ("ChilledWater Inlet Node", woCh.getString(Chiller_Electric_EIRFields::ChilledWaterInletNodeName).get());
    EXPECT_EQ("ChilledWater Outlet Node", woCh.getString(Chiller_Electric_EIRFields::ChilledWaterOutletNodeName).get());
    ...
    EXPECT_EQ("LeavingSetpointModulated", woCh.getString(Chiller_Electric_EIRFields::ChillerFlowMode).get());
    EXPECT_EQ(2.5, woCh.getDouble(Chiller_Electric_EIRFields::DesignHeatRecoveryWaterFlowRate).get());
    EXPECT_EQ("Heat Recovery Inlet Node", woCh.getString(Chiller_Electric_EIRFields::HeatRecoveryInletNodeName).get());
    EXPECT_EQ("Heat Recovery Outlet Node", woCh.getString(Chiller_Electric_EIRFields::HeatRecoveryOutletNodeName).get());
    EXPECT_EQ(1.8, woCh.getDouble(Chiller_Electric_EIRFields::SizingFactor).get());
    EXPECT_EQ(2000.0, woCh.getDouble(Chiller_Electric_EIRFields::BasinHeaterCapacity).get());
    EXPECT_EQ(5.0, woCh.getDouble(Chiller_Electric_EIRFields::BasinHeaterSetpointTemperature).get());
    EXPECT_EQ("basinHeaterOperatingSchedule", woCh.getString(Chiller_Electric_EIRFields::BasinHeaterOperatingScheduleName).get());
    EXPECT_EQ(0.97, woCh.getDouble(Chiller_Electric_EIRFields::CondenserHeatRecoveryRelativeCapacityFraction).get());
    EXPECT_EQ("heatRecoveryInletHighTemperatureLimitSchedule",
              woCh.getString(Chiller_Electric_EIRFields::HeatRecoveryInletHighTemperatureLimitScheduleName).get());
    EXPECT_EQ("HWLoop Supply Outlet", woCh.getString(Chiller_Electric_EIRFields::HeatRecoveryLeavingTemperatureSetpointNodeName).get());
    EXPECT_EQ("Chillers", woCh.getString(Chiller_Electric_EIRFields::EndUseSubcategory).get());
    EXPECT_EQ("ConstantFlow", woCh.getString(Chiller_Electric_EIRFields::CondenserFlowControl).get());
    EXPECT_EQ("condenserLoopFlowRateFractionFunctionofLoopPartLoadRatioCurve",
              woCh.getString(Chiller_Electric_EIRFields::CondenserLoopFlowRateFractionFunctionofLoopPartLoadRatioCurveName).get());
    EXPECT_EQ("temperatureDifferenceAcrossCondenserSchedule",
              woCh.getString(Chiller_Electric_EIRFields::TemperatureDifferenceAcrossCondenserScheduleName).get());
    EXPECT_EQ(0.9, woCh.getDouble(Chiller_Electric_EIRFields::CondenserMinimumFlowFraction).get());
    EXPECT_EQ("thermosiphonCapacityFractionCurve", woCh.getString(Chiller_Electric_EIRFields::ThermosiphonCapacityFractionCurveName).get());
    EXPECT_EQ(4.1, woCh.getDouble(Chiller_Electric_EIRFields::ThermosiphonMinimumTemperatureDifference).get());
    ...
    for (const auto& e : expecteds) {
      ...
      if (e.isSupply) {
        ...
        ASSERT_EQ("Chiller:Electric:EIR", peqs.front().getString(PlantEquipmentListExtensibleFields::EquipmentObjectType).get());
